...
COOKIES_TEXT = os.getenv("COOKIES", "")              # Paste Netscape cookie text into Render secret
...
def _write_cookies_to_temp() -> str | None:
    """Write cookies from env to a temp file and return its path."""
    if not COOKIES_TEXT:
        return None
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".txt")
    tmp.write(COOKIES_TEXT.encode())
    tmp.close()
    return tmp.name


def _ydl_opts(output_tmpl: str = "%(title)s.%(ext)s", cookiefile: str | None = None):
    opts = {
        'format': 'bestaudio/best',
        'noplaylist': True,
        'default_search': 'ytsearch10',  # fetch up to 10 entries for selection
        'outtmpl': output_tmpl,
        'quiet': True,
        'postprocessors': [
            {
...
async def _send_search_menu(update: Update, results: List[dict]):
    """Send a numbered list (1..10) and inline keyboard to pick."""
    lines = ["🔎 Search results:"]
    for i, it in enumerate(results, start=1):
        title = it.get('title') or 'Unknown title'
        uploader = it.get('uploader')
        if uploader:
            line = f"{i}. {title} — {uploader}"
        else:
            line = f"{i}. {title}"
        lines.append(line)
...
async def song_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("İstifadə: /song Mahnı adı — məsələn: /song Röya Bahar")
        return
